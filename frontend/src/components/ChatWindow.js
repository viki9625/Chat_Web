  ...
  const handleSendMessage = async (e) => {
    ...
    if (activeChat.type === "private") {
      ...
      apiPayload = {
        ...
        text: text.trim(),
        type: "private", // 👈 add type here
      };

      // make type top-level in websocket payload
      wsPayload = {
        event: "private_message",
        type: "private", // 👈 add top-level type
        sender: user.username,
        receiver: activeChat.id,
        text: text.trim(),
      };
      ...
      apiPayload = {
        ...
        room: activeChat.id,
        text: text.trim(),
        type: "room", // 👈 already correct
      };

      wsPayload = {
        event: "room_message",
        type: "room", // 👈 add top-level type
        sender: user.username,
        room: activeChat.id,
        text: text.trim(),
      };
  ...
  if (!activeChat) {
    return (
      <div className="chat-window-placeholder">
        <img
          src="https://img.icons8.com/plasticine/512/imessage.png"
          alt="Start chatting"
          className="placeholder-gif"
        />
        <h2>Select a conversation to begin</h2>
        <p>
          You can start a new chat by selecting a user or group from the list on
          the left.
        </p>
      </div>
    );
      ...
      <div className="message-list">
        {messages
          ...
          .map((msg, index, sortedMessages) => {
            ...
            return (
              <React.Fragment
                key={msg._id || msg._tempId || msg.timestamp + index}
              >
